...
class UNetGenerator(nn.Module):
    ...
    def forward(self, X, return_stats=False):
        ...
        # ── Shape decoder (unit variance normalized) ─────────────────────────
        out = []
        for c in range(self.C):
            h = bot[c]
        ...
        # Force unit-variance shape — prevents scale collapse
        # If shape_norm is flat, std→0 and normalization explodes, so clamp
        ...
        shape_norm = shape_dyn / shape_std              # (B, T, C) truly unit variance

        # Clamp pred_scale to prevent trivial collapse to zero
        # known_std gives a data-driven lower bound
        known_std_mean = known_std_flat.view(B, self.n_known, self.C).mean(dim=1)  # (B, C)
        pred_scale = pred_scale.clamp(min=known_std_mean * 0.1)

        # ── Reconstruct: shape * scale + dc ──────────────────────────────────
...
def train_translator(
    ...
    for k, ki in enumerate(known_stream_indices):
        name = stream_names[k] if stream_names else f"stream_{ki}"
    ...
    if encoders is not None and stream_names is not None and stream_to_encoder is not None:
        ...
        try:
            ...
            known_names = [stream_names[k] for k in known_stream_indices]
    ...
    history = {"epoch": [], "g_loss": [], "l1_loss": [],
                "dc_loss": [], "scale_loss": [], "feat_loss": [], "val_l1": []}
    ...
    for epoch in range(1, epochs + 1):
        ...
        g_loss_sum = 0.0
        l1_sum = dc_sum = scale_sum = feat_sum = embed_sum = 0.0
        nb = 0
        ...
        for i in range(0, N, batch_size):
            ...
            # Target DC and scale (supervision for the two heads)
            ...
            with torch.no_grad():
                ...
                w = (win_var / (win_var.mean() + 1e-8)).clamp(0.1, 40.0)
                w = w / w.mean()
            ...
            loss_scale = ((torch.log(pred_scale + 1e-6) -
                           torch.log(target_scale + 1e-6)).pow(2).mean(dim=-1) * w).mean()

            # ── Embedding loss: SimCLR(generated) ≈ SimCLR(real thigh) ──
            loss_embed = torch.tensor(0.0, device=DEVICE)
            if target_encoder is not None:
                with torch.no_grad():
                    z_real = target_encoder(X_target)          # (B, D) frozen
                z_gen  = target_encoder(reconstructed)         # (B, D) with grad
        ...
        history["scale_loss"].append(scale_sum / max(1, nb))
        ...
        if viz_dir and (epoch % viz_every == 0 or epoch == 1):
            ...
            # viz_samples has shape (N, T, n_known+1, C) — target is always last
            _visualize_translator(G, viz_samples, known_stream_indices,
                                  viz_samples.shape[2] - 1, n_known, T, C,
...
def _save_loss_curve(history, out_path):
    ...
    ax1.plot(ep, history["l1_loss"],   label="L1",        color="#4CAF50", lw=1.2, ls="--")
    ax1.plot(ep, history["dc_loss"],   label="DC",        color="#E53935", lw=1.2, ls="--")
    ax1.plot(ep, history["scale_loss"],label="Scale",     color="#FF9800", lw=1.2, ls="--")
...
def _visualize_translator(G, X_samples, known_indices, target_idx,
    ...
    with torch.no_grad():
        ...
        fake_norm = G(kn_norm)   # (B, T, C) raw generator output
...
def impute_with_translator(
    ...
    with torch.no_grad():
        for i in range(0, len(X_known), batch_size):
            ...
            # Extract unit-variance shape from generator output
            gen_dc   = gen_out.mean(dim=1, keepdim=True)
            gen_dyn  = gen_out - gen_dc
            gen_std  = gen_dyn.std(dim=1, keepdim=True).clamp(min=1e-6)
            gen_shape = gen_dyn / gen_std              # (B, T, C) unit variance
            ...
            if pw is not None:
                pw_t = torch.from_numpy(pw).to(DEVICE)
                sr_t = torch.from_numpy(sr).to(DEVICE)
                proxy_amp = torch.zeros(B_cur, C, device=DEVICE)
                for k in range(n_known):
                    kn_raw = bk[:, :, k, :]
                    kn_dyn = kn_raw - kn_raw.mean(dim=1, keepdim=True)
                ...
                proxy_amp = (bk - bk.mean(dim=1, keepdim=True)
            ...
            # ── Reconstruct: generator shape × proxy amplitude + DC MLP ──
            fake_denorm = gen_shape * proxy_amp.unsqueeze(1) + pred_dc.unsqueeze(1)
...
if __name__ == "__main__":
    ...
    cli_stream_names = args.initial_sensors + [args.target_sensor]
    try:
        import sys as _sys
        _sys.path.insert(0, str(Path(__file__).parent))
...
if __name__ == "__main__":
    ...
    if args.viz_only:
        ...
        viz_out = os.path.join(args.out_dir, "viz", "viz_only.png")
        Path(os.path.join(args.out_dir, "viz")).mkdir(parents=True, exist_ok=True)
        from signal_translator import _visualize_translator
        _visualize_translator(
            translator.generator, viz_samples,
            args.known_streams, args.target_stream,
            len(args.known_streams), T, C,
    ...
    try:
        import sys as _sys
        _sys.path.insert(0, str(Path(__file__).parent))
        from encoder import load_encoders_from_cfg
        from config_loader import cfg
        cli_encoders = load_encoders_from_cfg(cfg)
        cli_stream_to_encoder = cfg.STREAM_TO_ENCODER
        print(f"  [CLI] SimCLR encoders loaded for embedding loss", flush=True)
